...
??? example "Details"
    ...
    - [`Synth`](Synth.md): The fee pool, retrieving their addresses from the Oikos contract, directly burns and issues oUSD when transferring fees. Fees are denominated and paid out in oUSD. Synths themselves do not know the fee pool address directly, but ask the fee pool's proxy for its target.
...
Returns an array of [`FEE_PERIOD_LENGTH`](#fee_period_length) `[fees, rewards]` pairs owed to an account for each [recent fee period](#recentfeeperiods) (including the current one). Fees are denominated in oUSD and rewards in OKS.
...
??? example "Details"
    ...
    * [`FeesClaimed(claimingAddress, feesPaid, rewardsPaid)`](#feesclaimed) (`feesPaid` is denominated in oUSD, `rewardsPaid` in OKS)
